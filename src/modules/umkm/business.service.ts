...
export async function verifyBusinessOwnership(userId: string, businessId: string) {
  const business = await prisma.business.findFirst({
    ...
    select: { id: true, userId: true, name: true, type: true, address: true, lat: true, lng: true },
  });
...
export async function deleteBusiness(userId: string, businessId: string) {
  ...
  if (!existing) throw new AppError(404, UMKM_ERROR_MESSAGES.BUSINESS_NOT_FOUND);
